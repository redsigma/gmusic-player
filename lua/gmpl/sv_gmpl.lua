...
net.Receive("sv_play_live", function(length, sender)
    if not IsValid(sender) then return end
    print("Shared settings:", shared_settings)
    ...
    if shared_settings:get_admin_server_access() and
      not sender:IsAdmin() then
      return
    end
...
--]]
net.Receive("sv_refresh_access", function(length, sender)
  if not IsValid(sender) then return end
...
net.Receive("sv_reset_audio", function(length, sender)
  liveSong = ""
  liveSongIndex = 0
  liveSeek = 0
  isLooped = false
