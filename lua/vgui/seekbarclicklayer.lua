...
function PANEL:Init()
    ...
    self.Slider.OnMousePressed = function(mcode)
        self:OnMousePressed(mcode)
    end
    self.Slider.OnMouseReleased = function(mcode)
...
function PANEL:HasSliderEnded()
    -- override
end

function PANEL:MonitorSeek()
...
function PANEL:OnCursorMoved(pos_x)
    ...
    pos_x = math.Remap(self.seek_seconds_from_slider, 0, self:GetMax(), 0, 1)
    self:SetSlider(pos_x)
