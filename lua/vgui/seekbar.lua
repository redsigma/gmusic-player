...
function PANEL:Init()
    ...
    self.seek_val.AfterCursorMove = function(panel, seconds_at_cursor)
        self.seek_text:SetValue(string.ToMinutesSeconds(seconds_at_cursor))
    end
...
function PANEL:SetTextColor(color)
	textColor = color
	self.seek_text.Paint(self.seek_text, self.seek_text:GetWide(), self.seek_text:GetTall())
	self.seek_text_max.Paint(self.seek_text_max, self.seek_text_max:GetWide(), self.seek_text_max:GetTall())
...
function PANEL:ResetValue()
	self.seek_text:SetValue(string.ToMinutesSeconds(self:GetMin()))
...
function PANEL:SetTime(val)
    ...
    local pos_x = math.Remap(val, 0, max, 0, 1)
	self.seek_val:SetSeekSeconds(val)
	self.seek_text:SetValue(string.ToMinutesSeconds(val))
    self.seek_val:SetSlider(pos_x)
end
