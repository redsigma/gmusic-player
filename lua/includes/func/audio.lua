...
GMPL_AUDIO.new = function(self)
   ...
   local methods = {
        attrs = {
            ...
            isMissing = false,
            isAutoPlaying = false,
            isLivePaused = false, -- used for users to pause server
            error = false,
            seek = 0,
            prev_volume = 0,
            title_status = "",
        ...
        is_paused_live = function(self)
          return self.isLivePaused
        end,
        ...
        play = function(self, song, song_index, is_autoplay, is_loop, seek, callback)
          ...
          sound.PlayFile(song, "noblock noplay", function(CurrentSong, ErrorID, ErrorName)
            ...
            self.title_song =
              string.StripExtension(string.GetFileFromFilename(song))

            CurrentSong:SetTime((seek or 0))
            CurrentSong:SetVolume(
              (dermaBase.slidervol:GetVolume() / 100))
            ...
            dermaBase.contextmedia:SetSeekLength(
        ...
        IsValid = function(self)
          return channel ~= nil and IsValid(channel)
        end,
   }
      ...
      if status == 2 then
        ...
        if is_server_mode then
          if shared_settings:get_admin_server_access() then
            if channel:is_paused_live() then
              color_bg = colAPause
              color_text = colBlack
            else
              color_bg = colPause
...
local function kill_song(self)
    ...
    if dermaBase.main:IsServerMode() then
        ...
        self.sv_PlayingSong:silent_stop()
        -- sv_isStopped = true
        -- sv_isPaused = false
        -- timer.Stop("gmpl_sv_guard")
    else
        -- print("[core_kill] kill client")
        self.cl_PlayingSong:silent_stop()
        -- self.cl_PlayingSong = nil
        -- cl_isStopped = true
        -- cl_isPaused = false
        -- timer.Stop("gmpl_cl_guard")
...
local function songClientTime(self) return getMedia(self):GetTime() end
...
local function songTime(self)
  local chann = get_audio_channel(self)
  if not IsValid(chann) then return 0 end
  return chann:get_time()
end

local function volumeState(self) return getMedia(self):GetVolume() end
local function songVol(self, time) getMedia(self):SetVolume(time) end
--[[
...
local function uiPlay(self)
  local media = get_audio_channel(self)
...
local function playSong(self, song, song_index, is_autoplay, is_loop, seek)
  local media = self.cl_PlayingSong
  local mute_client_side = false
  if dermaBase.main:IsServerMode() then
    media = self.sv_PlayingSong
    mute_client_side = true
  end
  if media == nil then return end

  media:play(song, song_index, is_autoplay, is_loop, seek,
    function(is_valid)
      if not is_valid then return end

      local status = false
      if mute_client_side then
        self.cl_PlayingSong:mute()
      else
        self.sv_PlayingSong:mute()
      end
      if media:is_looped() then
        status = 3
      else
        status = 1
      end
      local color_bg, color_text = updateTitleColor(status, media)
      updateListSelection(color_bg, color_text, self)
      OnClientAudioChange(media)
    end)
...
local function play_next_song(self)
  if dermaBase.songlist:IsEmpty() then
    self:cl_stop()
    return
  end
  local song_list = dermaBase.song_data:get_current_list()
  local next_song_index = self.cl_PlayingSong:get_song_index() + 1
  if next_song_index > #song_list then
    next_song_index = 1
  end
  playSong(self, song_list[next_song_index], next_song_index, true)
...
local function playSongServer(self, song, song_index, is_autoplay, is_loop, seek)
  playSong(self, song, song_index, is_autoplay, is_loop, seek)
    ...
    --             end
    --             CurrentSong:SetTime((seek or 0))
    --             CurrentSong:SetVolume(dermaBase.slidervol:GetVolume())
    --             updateAudioObject(self, CurrentSong, true)
...
local function update_ui_selection(self, channel)
  ...
  if channel:is_paused() then
      color_bg, color_text = updateTitleColor(2, channel)
...
local function pause_live(self)
  if not IsValid(self.sv_PlayingSong) then return end
  return self.sv_PlayingSong:pause_live()
end
