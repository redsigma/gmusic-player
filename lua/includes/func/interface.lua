...
local function create_media_player()
	...
	dermaBase.sliderseek:GetSeekLayer().Slider.Knob:SetHeight(
        dermaBase.sliderseek:GetTall())
    ...
    if not dermaBase.cbadminaccess:GetChecked() then
      ...
      net.WriteDouble(dermaBase.mediaplayer:getTime())
      net.SendToServer()
      return
    end
    ...
    if LocalPlayer():IsAdmin() then
      ...
      if dermaBase.mediaplayer:is_paused_live() then
	...
	dermaBase.buttonplay.DoClick = function(self, song_path, line_index)
        if dermaBase.songlist:IsEmpty() then
            return
        elseif not isnumber(line_index) then
          local last_line_index =
            dermaBase.mediaplayer:get_channel():get_song_index()
        ...
        if not isstring(song_path) then
            song_path = dermaBase.song_data:get_song(line_index)
        end
        if dermaBase.main:IsServerMode() then
            ...
            if dermaBase.mediaplayer:hasValidity() and
                ...
                dermaBase.mediaplayer:resume(song_path, line_index)
            else
