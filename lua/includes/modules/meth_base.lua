...
local callbacks = {}
callbacks.OnClientAudioChange = function(media)
...
local function init(contextMenu, contextMargin)
  ...
  dermaBase.mediaplayer =
    include("includes/modules/musicplayerclass.lua")(dermaBase, callbacks)
  ...
  dermaBase.main.OnClientMode = function()
    dermaBase.mediaplayer:ToggleNormalUI()
	end
  dermaBase.main.OnServerMode = function()
    if dermaBase.cbadminaccess:GetChecked() and
      not LocalPlayer():IsAdmin() then
  ...
  dermaBase.sliderseek.seek_val.OnEndReached = function()
    if dermaBase.main:IsServerMode() then
        ...
        if dermaBase.mediaplayer:is_looped() then
          ...
          print(dermaBase.sliderseek:GetMin())
  ...
  -- Used for stopping the realtime audio after checking slider pos 3 times
  dermaBase.sliderseek.seek_val.HasSliderEnded = function(panel)
    local state = dermaBase.mediaplayer:get_channel():state()
    ...
    if panel.current_slider_pos == panel.previous_slider_pos
  ...
  dermaBase.buttonstop.DoClick = function()
    ...
    dermaBase.painter.update_colors = function(self)
        ...
        if dermaBase.mediaplayer:hasValidity() and
            dermaBase.mediaplayer:is_looped() then
