...
local function realtime_seek(self)
  ...
  if dermaBase.main:IsVisible() or
    dermaBase.mediaplayer:is_autoplaying() then
    ...
    elseif self:is_looped() then
      dermaBase.sliderseek:AllowSeek(true)
      dermaBase.sliderseek:SetTime(
        dermaBase.sliderseek:GetMin(), dermaBase.sliderseek:GetMin())
    end
	end
end
...
function Media:net_init()
  net.Receive("cl_stop_live", function(length, ply)
    ...
    net.Receive("cl_play_live", function(length, ply)
      ...
      if dermaBase.main:IsServerMode() then
        ...
        dermaBase.mediaplayer:play(
          live_song, index_song, is_autoplayed, is_looped)
        -- else
    ...
    net.Receive("cl_set_autoplay", function(length, ply)
      if not dermaBase.mediaplayer:hasValidity() or
        not dermaBase.main:IsServerMode() then return end
    ...
    net.Receive("cl_pause_live", function(length, ply)
    ...
    net.Receive("cl_play_live_seek_from_host", function(length, ply)
      local user = net.ReadEntity()
    ...
    net.Receive("cl_play_live_seek", function(length, ply)
        ...
        dermaBase.mediaplayer:play_server(
